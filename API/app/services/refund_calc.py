...
[증거 인용 규칙]
- 모든 수치(자기부담률, 공제, 한도, 횟수 등)와 결정 문장 뒤에는 대괄호 인용키를 붙인다. 예: "자기부담 30%[DB1]", "연간 5,000만원[DB1]", "비급여 제외[RC2]".
- 인용키 체계: [DBn] = [DB COVERAGE]의 n번째 항목(위에서부터 1부터), [RCn] = [RAG CONTEXT]의 n번째 인용. [ANn] = [RAG AUTO ANSWER] 참고용(단독 인용 금지).
- 답변 마지막의 '🔎 출처' 섹션에서 각 키를 보험사/상품/항목/원문요지로 풀어쓴다. 제너릭 문구 금지.

[계산 공개 규칙]
- 반드시 "계산과정"을 출력: 수식 → 대입값 → 한도/공제/자기부담 적용 순서.
- 기본식 예시: min(연간한도, 1회한도, max(0, 청구금액 − 공제) × (1 − 자기부담률))[DBx].
...
[출처 섹션 작성 규칙]
- 최소 1개 이상의 [DBn] 또는 [RCn]가 있어야 하며, 없으면 규칙 3을 따른다.
- 각 출처는 한 줄로 요약한다: "[DB1] 흥국화재/실손/외래·비급여 자기부담 30%, 공제 30,000원, 1회 200,000원, 연 50,000,000원 (문서/조항 식별)".
...
🔎 출처: {각 [DBn]/[RCn]/[ANn] 키에 대한 1줄 요약 목록}
""".strip()
...
def build_messages(user_text: str, context: str = "") -> List[Dict[str, str]]:
    ...
    if ctx.strip():
        ...
        source_instruction = (
            "컨텍스트의 [DB COVERAGE] 각 항목을 위에서부터 [DB1], [DB2], …로 간주하여 인용키를 붙이세요. "
            "[RAG CONTEXT] 문단은 [RC1], [RC2] …로 인용하세요. [RAG AUTO ANSWER]는 [AN1]…로 보조설명에만 사용하고 단독 인용은 금지합니다. "
